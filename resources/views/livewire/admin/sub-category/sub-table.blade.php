                      ...
                      <div class="px-6 py-1.5">
                          <div class="hs-dropdown relative inline-block [--placement:bottom-right]">
                              ...
                              <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden mt-2 divide-y divide-gray-200 min-w-[10rem] z-10 bg-white shadow-xl rounded-lg border border-gray-100" aria-labelledby="hs-table-dropdown-1">
                                  <div class="p-1">
                                      <button type="button" class="w-full flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100" @click="$dispatch('dispatch-subcategory-table-edit', { id: '{{ $category->id }}' })">
                                          Edit
                                      </button>
                                  </div>
                                  <div class="p-1">
                                      <button type="button" class="w-full flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-red-600 hover:bg-gray-100" @click="$dispatch('dispatch-subcategory-table-delete', { id: '{{ $category->id }}', name: '{{ $category->name }}' })">
                                          Delete
                                      </button>
                                  </div>
                              </div>
